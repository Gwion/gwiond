...
static SYMBOL_ENTRY symbol_table[SYMBOL_TABLE_LENGTH];
int first_empty = 0;
...
int gwiond_insert_symbol(char *name,
    char *_type,
    unsigned kind,
    unsigned type,
    unsigned atr1,
  ...
  symbol_table[index]._type = _type;
...
char* lookup_symbol_type(const char *name, unsigned kind) {
  int i;
  for(i = first_empty - 1; i > FUN_REG; i--) {
    if(strcmp(symbol_table[i].name, name) == 0
       && symbol_table[i].kind & kind)
       return symbol_table[i]._type;
  }
  return NULL;
}
...
void set_range(int index, loc_t range) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    symbol_table[index].range = range;
}

loc_t get_range(int index) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    return symbol_table[index].range;
  loc_t no_range = NO_RANGE;
  return no_range;
}

char* get_display(int index) {
// return value type
return strdup("oikhk hkj");
...
void clear_symbols(int begin_index) {
  ...
  if(begin_index > first_empty) {
    fprintf(stderr, "Compiler error! Wrong clear symbols argument");
    exit(EXIT_FAILURE);
  }
...
void init_symtab(void) {
  ...
  for(i = 0; i < 14; i++) {
    sprintf(s, "%%%d", i);
    loc_t no_range = NO_RANGE;
    gwiond_insert_symbol(strdup(s), "", REG, NO_TYPE, NO_ATR, NO_ATR, no_range);
  }
}
